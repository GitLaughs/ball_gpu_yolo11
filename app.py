...
def detection_loop():
    ...
    try:
        cfg = state.cfg
        print(f"[INFO] Loading stereo params...")
        stereo = build_stereo_params(cfg.width, cfg.height)

        print(f"[INFO] Loading YOLO model from: {state.model_path}")
        if not os.path.exists(state.model_path):
            print(f"[ERROR] Model file not found: {state.model_path}")
            with state.lock:
        ...
        print(f"[INFO] Opening video: {state.video_path}")
        if not os.path.exists(state.video_path):
            print(f"[ERROR] Video file not found: {state.video_path}")
            with state.lock:
                state.is_running = False
            return
        ...
        if not cap.isOpened():
            print(f"[ERROR] cv2.VideoCapture failed to open: {state.video_path}")
            with state.lock:
                state.is_running = False
            return
        ...
        print(f"[INFO] Video opened: {vid_w}x{vid_h}, "
              f"expected stereo side-by-side: {cfg.width * 2}x{cfg.height}")
        if vid_w < cfg.width * 2:
            print(f"[WARN] Video width {vid_w} < {cfg.width * 2}. "
                  f"process_frame expects side-by-side stereo! May crash.")
        ...
        while True:
            ...
            ok, frame = cap.read()
            if not ok:
                print(f"[INFO] Video ended at frame {frame_id}")
                with state.lock:
                    state.is_running = False
                break
            ...
            try:
                ...
                print(f"[ERROR] process_frame failed at frame {frame_id}: {e}")
                import traceback; traceback.print_exc()
                continue  # 跳过这帧，不要崩溃整个循环
            ...
            with state.lock:
                ...
                state.last_shot["current_shot_sequence"] = []
        ...
        cap.release()
        print("[INFO] detection_loop finished normally")

    except Exception as e:
        import traceback
        print(f"[FATAL] detection_loop crashed: {e}")
        traceback.print_exc()
    finally:
        with state.lock:
            state.is_running = False
        ...
        print("[INFO] detection_loop thread exiting")
