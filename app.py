...
def build_stereo(w: int = 640, h: int = 480) -> StereoParams:
    lm = np.array([[834.45170, 0, 341.14720],
                   [0, 834.98848, 239.35073],
                   [0.,        0.,        1.]])
    rm = np.array([[838.79197, 0, 326.20140],
    ...
    R  = np.array([[ 0.999953933, -0.005218517,  0.008055977],
...
class AppState:
    def __init__(self):
        ...
        self.model_path  = ("./best_yolo11.pt"
...
def detection_loop():
    try:
        ...
        stable_dets, total_frames, vid_fps = preprocessor.process(
            state.video_path, progress_cb)
        ...
        ball_frames = sum(1 for v in stable_dets.values()
                         if any(d.cls == "basketball" for d in v))
        hoop_frames = sum(1 for v in stable_dets.values()
                         if any(d.cls == "hoop" for d in v))
        print(f"[INFO] 预处理完成: 共 {det_count} 个检测 | "
        ...
        print(f"[INFO] 开始回放: {total_frames} 帧 @ {vid_fps:.1f} fps")
        ...
        while True:
            ...
            try:
                left_draw, right_draw, depth_color, fd = vision.process_frame(
                    frame, frame_id, dets_2d,
            ...
            for d in fd.detections:
                if d.cls == "basketball" and (ball_det is None
                                              or d.conf > ball_det.conf):
                    ball_det = d
                if d.cls == "hoop" and (hoop_det is None
                                        or d.conf > hoop_det.conf):
                    hoop_det = d

            ts_now     = time.time()
            shot_event = shot_engine.update(frame_id, ts_now,
                                            ball_det, hoop_det)

            bc   = sum(1 for d in fd.detections if d.cls == "basketball")
            hc   = sum(1 for d in fd.detections if d.cls == "hoop")
            proc = time.time() - t0
            fps_smooth = 0.9 * fps_smooth + 0.1 * (1.0 / max(proc, 1e-4))
            ...
            with state.lock:
                ...
                state.shot_info["total_scored"] = shot_engine.total_scored
                if shot_engine.total_shots > 0:
                    pct = shot_engine.total_scored / shot_engine.total_shots * 100
                ...
                if shot_event is not None:
                    ...
                    if len(state.shot_info["shot_history"]) > 20:
                        state.shot_info["shot_history"] = \
                            state.shot_info["shot_history"][-20:]

                    tag = "✓ 命中" if shot_event.is_scored else "✗ 未中"
                    print(f"[投篮] #{shot_engine.total_shots:02d} {tag} | "
                          f"弧高={shot_event.arc_height_px:.0f}px | "
                          f"时长={shot_event.duration_s:.2f}s | "
                          f"帧段 {shot_event.frame_start}-{shot_event.frame_end}")
...
def start_detection():
    with state.lock:
        ...
        state.shot_info  = {
            ...
            "accuracy": "0%", "last_shot": None, "shot_history": [],
        }
...
def gen_frames(ftype: str = "left"):
    ...
    while True:
        ...
        if f is None:
            ...
            with state.lock:
                msg = (state.preprocess_progress or "预处理中..."
                       ) if state.is_preprocessing else "等待视频..."
            cv2.putText(blank, msg, (30, 240),
